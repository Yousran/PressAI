...
<div class="modal fade" id="tambah_ujian" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
        <div class="modal-header">
            <h1 class="modal-title fs-5" id="exampleModalLabel">Tambah Ujian</h1>
        ...
        <form action="{{ route('test.create') }}" method="post">
            <div class="modal-body">
                ...
                <div class="mb-3">
                    <label class="form-label">Durasi Ujian</label>
...
<div class="card m-3">
    <div class="card-body">
        ...
        <div class="d-flex justify-content-end">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#tambah_ujian">Tambah Ujian</button>
        </div>
        <div class="table-responsive">
            <table id="datatable" class="table" style="width:100%">
                ...
                <tbody>
                    @if ($mata_kuliah&&isset($mata_kuliah['test']))
                    @foreach ($mata_kuliah['test'] as $test)
                        <tr>
                            <td>{{$test['test_name']}}</td>
                            <td>{{$test['tanggal_test']}}</td>
                            <td>
                                {{$test['awal_waktu']}} - {{$test['akhir_waktu']}}
                            </td>
                            <td>
                                ...
                                <div class="btn-group">
                                        ...
                                        <a href="{{ route('test.generateqr', ['test_code' => $test['test_code']]) }}" class="btn btn-info">Tampilkan QR</a>
                                        <a href="{{ route('soal./', ['test_code' => $test['test_code']]) }}" class="btn btn-info">Lihat Soal</a>
                                        @csrf
                                        <input type="hidden" name="code" value="{{ $mata_kuliah['mata_kuliah_code'] }}">
